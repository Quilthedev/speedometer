	...
	while true do
		...
		if(IsPedInAnyVehicle(Ped)) then
			...
			if PedCar and GetPedInVehicleSeat(PedCar, -1) == Ped then
				...
				if IsControlJustPressed(1, 314) then -- F9 is pressed
					ind.l = not ind.l
					SetVehicleIndicatorLights(GetVehiclePedIsUsing(GetPlayerPed(-1)), 0, ind.l)
				end
				if IsControlJustPressed(1, 315) then -- F10 is pressed
					ind.r = not ind.r
		...
		if(IsPedInAnyVehicle(Ped)) then
			...
			if PedCar and GetPedInVehicleSeat(PedCar, -1) == Ped then
				...
				elseif rpm > 0.7 then
					...
					Citizen.Wait(2000)
				elseif rpm > 0.6 then
					rpmfuel = fuel - rpm / 11.1
					Citizen.Wait(3000)
				elseif rpm > 0.5 then
					rpmfuel = fuel - rpm / 10.7
					Citizen.Wait(4000)
				elseif rpm > 0.4 then
					rpmfuel = fuel - rpm / 12.4
					Citizen.Wait(5000)
				elseif rpm > 0.3 then
					rpmfuel = fuel - rpm / 13.9
					Citizen.Wait(6000)
				elseif rpm > 0.2 then
					rpmfuel = fuel - rpm / 14.3
					Citizen.Wait(8000)
				else
					rpmfuel = fuel - rpm / 15.5
					Citizen.Wait(15000)
				end
